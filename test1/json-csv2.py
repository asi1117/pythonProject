...
datafile = ['data/review_322770.json','data/review_269170.json','data/review_412740.json']
csvfile = ['data/review_1.csv','data/review_2.csv','data/review_3.csv']
k =0

for i in datafile:
    print(i)
    print(csvfile[k])
    json_fp = open(i, "r", encoding='utf-8_sig')
    ...
    lables = ['recommendationid', 'language', 'review', 'anthor_steamid', 'anthor_num_games_owned',
              'anthor_num_reviews', 'author_playtime_forever', 'author_playtime_at_review', 'author_last_played',
              'timestamp_created', 'timestamp_updated', 'voted_up', 'votes_up', 'votes_funny', 'weighted_vote_score',
              'comment_count', 'steam_purchase', 'received_for_free', 'written_during_early_access']
    print(list_skey)
    with open(csvfile[k], 'w', encoding='utf-8_sig') as f:
        writer = csv.writer(f)
        writer.writerow(lables)
        for i in list_skey:
            # k=list_skey[ixkongzhi
            ...
            wdata = [i, dict_language, dict_review, dict_steamid, dict_num_games_owned, dict_num_reviews,
                     dict_playtime_forever, dict_playtime_last_two_weeks, dict_playtime_at_review, dict_last_played,
                     dict_timestamp_created,
                     dict_timestamp_updated, dict_voted_up, dict_votes_up, dict_votes_funny, dict_weighted_vote_score,
                     dict_comment_count, dict_steam_purchase, dict_received_for_free, dict_written_during_early_access]
            writer.writerow(wdata)  # 写入是个列表
    k += 1 #将 要创建的csv文件也放到列表里，采用+1用下标取出
